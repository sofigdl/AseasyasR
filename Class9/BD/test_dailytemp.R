#Link of the desired data files (daily precipitation)
http1 <- "ftp://opendata.dwd.de/climate_environment/CDC/grids_germany/daily/Project_TRY/air_temperature_mean/"
library(RCurl)
result1<-getURL(http1, verbose=TRUE, ftp.use.epsv=TRUE, dirlistonly=TRUE)

#Split string into pieces
library(tidyverse)
result_tidy<-str_split(result1, "\n|\r\n")
...
result_tidy <- result_tidy[c(seq(1,216, by=1))]

setwd("Class9/BD/")

dir.create("Class9/BD/")
out_dir<-"Class9/BD"
...
mypath<-"Class9/BD/"
temp<-grep("*daymean*", list.files(path = mypath, pattern = "*.gz$"), value = T)
temp
